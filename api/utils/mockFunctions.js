...
exports.createCompleteAnime = async function() {
    ...
    if(res === mockAnime)
        console.log("Anime saved with success !");
    /*
    ...
    if(res === mockTorrentFile)
        console.log("Torrent file saved with success !");
    ...
    if(res === mockDownload)
        console.log("Download saved with success !");
    ...
    if(res === mockAnimeFile)
        console.log("Anime file saved with success !");

    res = await Download.updateOne({ _id: mockDownload._id}, {downloadedFile: mockAnimeFile._id, fileQuality: '1080p', status: 'finished'}).catch(err => {
        if(err) return console.error(err);
    });
    ...
    if(res === mockAnimeEpisode)
        console.log("Anime episode file saved with success !");
